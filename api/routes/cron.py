...
async def daily_summary_cron(
    ...
    if not settings.telegram_group_chat_ids:
        raise HTTPException(status_code=500, detail="TELEGRAM_GROUP_CHAT_ID is not configured.")
    ...
    for chat_id in settings.telegram_group_chat_ids:
        ...
        logger.info("cron daily-summary sent chat=%s day=%s", report.chat_id, report.day_key)
